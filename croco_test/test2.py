...
def gen_cluster(x, y):
    ...
    lat = np.zeros((l*n2))
    for i in range(l):
        lon0, lat0 = np.meshgrid(np.linspace(x[i]-dx, x[i]+dx, n), \
                               np.linspace(y[i]-dx, y[i]+dx, n))
        lon[(n2*i):(n2*(i+1))] = lon0.flatten()
        lat[(n2*i):(n2*(i+1))] = lat0.flatten()
    
    return lon, lat   

# File location
loc = '../../croco_sim/Run_WIOMonClim_Coburg10/CROCO_FILES/croco_24h.nc'
...
lons = np.array([55.36, 55.80, 46.18, 50.59, 57.69, 41.00, 72.4, 59.61, 45.14, 53.33])
lats = np.array([-4.56, -4.24, -9.33, -15.38, -19.96, -14.65, -7.48, -16.63, -13.04, -5.43])
lon0, lat0 = gen_cluster(lons, lats)
...
fig, ax = tracpy.plotting.background(roms_grid, fig=fig, extent=[35, 75, -20, 0],
